...
export default function DayBlock(props: DayBlockProps) {
	return (
		<TouchableOpacity onPress={() => { props.onPress(props.index) }} style={styles.column}>
			<Text style={[styles.lessonsText, styles.dayText]}>{(moment(props.item.date).format("ddd Do"))}</Text>
			...
			<Text style={[styles.lessonsText, styles.rangeText]}>{moment(props.item.timetable[0].date).format("H.mm")}</Text>
